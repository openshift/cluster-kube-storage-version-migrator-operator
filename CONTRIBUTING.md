...
./cluster-kube-storage-version-migrator-operator-tests-ext run-test \
  --test "should ensure NetworkPolicies are defined in the operand namespace"
...
./cluster-kube-storage-version-migrator-operator-tests-ext list tests \
  --suite=openshift/cluster-kube-storage-version-migrator-operator/operator/parallel
```
...
As an example, `launch 4.23,https://github.com/openshift/cluster-kube-storage-version-migrator-operator/pull/123 aws` would launch an OpenShift 4.23 cluster with the changes made in this PR running on AWS.
