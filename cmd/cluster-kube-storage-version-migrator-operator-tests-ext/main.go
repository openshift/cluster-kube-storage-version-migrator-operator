...
import (
	...
	// The import below is necessary to ensure that the kube storage version migrator operator tests are registered with the extension.
	_ "github.com/openshift/cluster-kube-storage-version-migrator-operator/test/extended"
)

func main() {
	registry := e.NewRegistry()
	ext := e.NewExtension("openshift", "payload", "cluster-kube-storage-version-migrator-operator")
	...
	// "[sig-kube-storage-version-migrator] <test name here>",
	)
	...
	root := &cobra.Command{
		Long: "Cluster Kube Storage Version Migrator Operator Tests Extension",
	}
